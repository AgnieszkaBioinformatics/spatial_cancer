...
metadata <- matrix(,length(colnames(merged_count_matrix)), 11)
...
for (i in 1:nrow(metadata)){
  gsm <- rownames(metadata)[i]
  gsm <- substr(gsm, 11, 20)
  index <- which(geo_metadata[["GSE135922_series_matrix.txt.gz"]]@phenoData@data[["geo_accession"]]==gsm)
  metadata[i, "GSM"] <- gsm
  patient_number <- geo_metadata[["GSE135922_series_matrix.txt.gz"]]@phenoData@data[["characteristics_ch1.1"]][index]
  patient_number <- sub("donor: ", "", patient_number)
  metadata[i, "Patient"] <-paste("patient", patient_number, sep=" ")
  metadata[i, "Region"] <- geo_metadata[["GSE135922_series_matrix.txt.gz"]]@phenoData@data[["location:ch1"]][index]
  if (patient_number=="1"){
    metadata[i, "Age"] <- "54"
    metadata[i, "Sex"] <- "male"
